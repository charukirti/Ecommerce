...
import { useFetchWishlistItem } from "../hooks/wishlist/useFetchWishlistItem";
import { useUpdateWishlist } from "../hooks/wishlist/useUpdateWishlist";
import { useDeleteWishlistItem } from "../hooks/wishlist/useDeteleWishlistItem";
...
export default function WishlistItem({
  ...
  const { data: wishlistItem, isLoading } = useFetchWishlistItem(productId);
  const { updateWishlist, isPending } = useUpdateWishlist();
  const { deleteItem, isPending: isDeleting } = useDeleteWishlistItem();
  ...
  return (
    <article className="flex items-center gap-4 p-5 border rounded-lg shadow-sm bg-white hover:shadow-md transition-shadow duration-200">
      <img
        src={wishlistItem?.product_image || "fallback.svg"}
        alt={`${wishlistItem?.product_name}`}
